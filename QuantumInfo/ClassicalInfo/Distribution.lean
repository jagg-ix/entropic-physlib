...
We define the type `Distribution α` on a `Fintype α`. By restricting ourselves to distributions
on finite types, a lot of notation and casts are greatly simplified. This suffices for (most)
...
/-- Given a distribution on α, extend it to a distribution on `Sum α β` by giving it no
  support on `β`. -/
def extend_right (d : ProbDistribution α) : ProbDistribution (α ⊕ β) :=
  ⟨fun x ↦ Sum.casesOn x d.val (Function.const _ 0), by simp⟩

/-- Given a distribution on α, extend it to a distribution on `Sum β α` by giving it no
  support on `β`. -/
...
afforded by the `Mixable` instance,
...
/-- The expectation value of a random variable with constant probability
  distribution `constant x` is its value at `x` -/
...
/-- Given a `T`-valued random variable `X` over `α`, mapping over `T` commutes with
  the equivalence over `α` -/
