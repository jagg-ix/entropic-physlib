...
TODO "6VZ3C" "In `Physlib/Relativity/Tensors/Basic.lean`, prove that for index permutations
  used by `TensorSpecies` tensors, if `σ : Fin n1 → Fin n` satisfies
  `PermCond c c1 σ` (with `h` its bijectivity component) then the inverse
  `PermCond.inv σ h` satisfies `PermCond c1 c (PermCond.inv σ h)`."
