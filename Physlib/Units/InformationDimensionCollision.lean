...
* `boltzmann_collides_thermoEntropy` — the physically relevant collision: without
  ...
  a forced collision: any theory containing an information-with quantity collapses
...
distinct dimensions (`k_B` records `I⁻¹`, `S` does not). So the 5-base SI structure
...
sources) — so every information-with quantity has a collision partner once `[I]`
is dropped. -/
