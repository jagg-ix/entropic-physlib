...
complex action is an **action-dimensioned scalar**: both `S_R` and `S_I` have dimension `E·T`.
...
quantum — has no dimensional exponent: `[i] = 1`. The dimension `I𝓭` belongs to
the imaginary *action* `S_I` (`imaginaryAction_dim`), not the *unit* `i`
...
has the information dimension `[I]`, giving the complex action a nonzero information
...
is the physical input, this is the mathematical backstop. -/
...
`[i] = 1 ≠ I𝓭 = [S_I]`. The unit has the dimensionless value `ln 2`; the action
has dimension `[I]`. -/
...
/-- **The scalar imaginary action has zero information**: `[S_I].information = 0`.
...
action has information exponent `1` (assigned to `S_I`), angular momentum has `0`. One
